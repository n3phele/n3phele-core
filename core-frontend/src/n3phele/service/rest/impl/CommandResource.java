...
public class CommandResource {
	...
	@RolesAllowed("authenticated")
	@Path("export")
	public CommandDefinitions export(
			@DefaultValue("false") 	@QueryParam("preferred") boolean preferred,
			@DefaultValue("0") 		@QueryParam("start") int start,
			@DefaultValue("-1") 	@QueryParam("end") int end,
			@DefaultValue("0") 	@QueryParam("clone") int clone,
									@QueryParam("search") String search) {
			...
			if(search != null) {
				...
				for(Command item : cmds.getElements()) {
					...
					if((nameMatch =item.getName().toLowerCase(Locale.ENGLISH).contains(search)) || 
					   ...
					   (descriptionMatch=item.getDescription().toLowerCase(Locale.ENGLISH).contains(search))) ||
					   (item.getOwner()!=null && 
							   (ownerMatch=item.getOwner().toString().toLowerCase(Locale.ENGLISH).contains(search))) ||
							   (item.getVersion()!=null && 
									   (versionMatch=item.getVersion().toLowerCase(Locale.ENGLISH).contains(search)))
					) {
						log.info("Adding "+item.getName()+" under "+search+" nameMatch "+nameMatch+" descriptionMatch "+descriptionMatch+" ownerMatch "+ownerMatch+" versionMatch "+versionMatch);
						filtered.add(item);
					}
				}
...
//						CommandDefinitions a =   (CommandDefinitions) u.unmarshal(stream);
						...
						for(CommandDefinition cd : a.getCommand()) {
							...
							if(existing == null) {
								...
								boolean mismatchedUri = (cd.getUri() != null && cd.getUri().toString().length()!=0) && !cd.getUri().equals(existing.getUri());
								log.info("requestor is "+requestor+" owner is "+existing.getOwner()+" isOwner "+isOwner);
								log.info("URI in definition is "+cd.getUri()+" existing is "+existing.getUri()+" mismatchedUri "+mismatchedUri);
	...
	public Response add(AddCommandRequest request) {
		...
		Command result = new Command(request.getName(), request.getDescription(), 
				...
				request.getVersion(), request.getApplication(), 
				inputs,
				executionParameters,
				outputs,
	...
	public Response addCloudProfile(@PathParam("Id") Long id,
			CloudProfile profile) throws Exception {
		...
		return Response.created(uriInfo.getAbsolutePathBuilder().uri(item.getUri()).path("cloudProfiles").path(Long.toString(profile.getId())).build()).build();
	}
	@GET
	...
	public CloudProfile getCloudProfile(@PathParam("Id") Long id,
	...
	private String addCommand(CommandDefinition cd) {
		...
		Command result = new Command(cd.getName(), cd.getDescription(), 
				UserResource.toUser(securityContext).getUri(), cd.isPublic(), cd.isPreferred(),
				cd.getVersion(), cd.getIcon(), 
				inputs,
				executionParameters,
				outputs,
				outputParameters, null);
	...
	public static class CommandManager extends CachingAbstractManager<Command> {
		...
		public List<Command> getAllPreferredPublic() {
			...
			try {
				result = itemDao.ofy().query(itemDao.clazz).filter("preferred", true).filter("isPublic", true).order("name").list();
			} catch (NotFoundException e) {
			}
			return result;
		}
		...
		public Collection<Command> getPreferredCollection(int start, int n) {
			...
			try {
				...
				List<Key<Command>> keys = itemDao.ofy().query(itemDao.clazz).order("name").filter("preferred", true).offset(start).limit(n).listKeys();
		...
		public Collection<Command> getPreferredCollection(URI owner) {
			Collection<Command> result = null;
			try {
				List<Command> owned = itemDao.ofy().query(itemDao.clazz).filter("owner", owner.toString()).filter("preferred", true).list();
				List<Command> shared = itemDao.ofy().query(itemDao.clazz).filter("preferred", true).filter("isPublic", true).list();
		...
		public Collection<Command> getPreferredCollection(URI owner, int start, int n) {
			...
			int owned = 0;
			try {
				owned = itemDao.ofy().query(itemDao.clazz).filter("owner", owner.toString()).filter("preferred", true).filter("isPublic", false).count();
				log.info("Owned is "+owned);
				if(owned > start) {
					list = itemDao.ofy().query(itemDao.clazz).order("name").filter("owner", owner.toString()).filter("preferred", true).filter("isPublic", false).offset(start).limit(n).list();
				...
				if(list == null || list.size() < n) {
					...
					List<Command> shared = itemDao.ofy().query(itemDao.clazz).order("name").filter("preferred", true).filter("isPublic", true).offset(start).limit(n).list();
					if(list == null) {
						list = shared;
					} else {
						list.addAll(shared);
